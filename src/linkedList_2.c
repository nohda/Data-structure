...
void delete(LinkedList *linkedList, int position){
    linkedList->current = linkedList->head;
    Node *pre = NULL;
    // Node *object = NULL;
...
int main(){
    ...
    delete(linkedList,newDelPos);
    for(int j =0; j<sizeof(linkedList->length); j++){
        printf("%d ", linkedList->current->data);
        linkedList->current = linkedList->current->next;
    }
    printf("\n");
